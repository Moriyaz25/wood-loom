...
export default function ProductMediaUploader({ value, onChange }) {
  ...
  return <div className="rounded-2xl border border-[#4a2816]/15 bg-white p-4">
    <div className="rounded-xl border-2 border-dashed border-[#d46b25]/35 bg-[#fff4e8] p-5 text-center"><input id="product-media" type="file" multiple accept="image/jpeg,image/png,image/webp,video/mp4,video/webm,video/quicktime" onChange={(event) => upload(event.target.files || [])} className="sr-only" /><label htmlFor="product-media" className="inline-flex cursor-pointer rounded-full bg-[#21150f] px-5 py-2.5 font-body text-xs font-semibold text-white hover:bg-[#d46b25]">+ Add photos or video</label><p className="mt-2 font-body text-[11px] font-medium text-[#765344]">Up to 8 media · images 8 MB · video 20 MB</p><p className="mt-1 font-body text-[11px] font-semibold text-[#c55312]">{state}</p></div>
